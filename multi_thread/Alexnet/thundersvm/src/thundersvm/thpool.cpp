...
#include <sys/prctl.h>
#endif
// under is the original
// #include "thpool.h"
#include "thundersvm/thpool.h"
#include "thundersvm/net.h"
//#include "utils.h"
#include <sched.h>
#include <sched.h>
...
struct thpool_* thpool_init(int num_threads, int n_all){
	...
	if (jobqueue_init(&thpool_p->jobqueue) < 0)
	{
...
/* ============================ HIDDEN LAYER OUTPUT QUEUE =========================== */

hiddenqueue* hiddenqueue_init(){
	hiddenqueue *hiddenqueue_p = (hiddenqueue *)malloc(sizeof(hiddenqueue));
	// Priqueue *priqueue = (Priqueue *) malloc(sizeof(Priqueue)) MPANIC(priqueue);
    hiddenqueue_p->front = hiddenqueue_p->rear = NULL;
    hiddenqueue_p->count = 0;
	return hiddenqueue_p;
	/* hiddenqueue mutex, cond init */
	// hiddenqueue_p->hidmutex = PTHREAD_MUTEX_INITIALIZER;
	// hiddenqueue_p->hid_cond = PTHREAD_COND_INITIALIZER;
}

int hiddenqueue_empty(hiddenqueue *hiddenqueue_p){
    return hiddenqueue_p->count == 0;
}

void hiddenqueue_push(hiddenqueue *hiddenqueue_p, torch::jit::IValue data){
    output *now = (output *)malloc(sizeof(output));
    now->data = data;
    now->next = NULL;
	std::cout<<hiddenqueue_p->count<<std::endl;

    if (hiddenqueue_empty(hiddenqueue_p)){
        hiddenqueue_p->front = now;
    }
    else{
        hiddenqueue_p->rear->next = now;
    }
    hiddenqueue_p->rear = now;
    hiddenqueue_p->count++;
}

torch::jit::IValue hiddenqueue_pull(hiddenqueue *hiddenqueue_p){
	torch::jit::IValue re = 0;
    // at::Tensor re;
    output *now;
    if(hiddenqueue_empty(hiddenqueue_p)){
        return re;
    }
    now = hiddenqueue_p->front;
    re = now->data;
    hiddenqueue_p->front = now->next;
    free(now);
    hiddenqueue_p->count--;
    return re;
}
