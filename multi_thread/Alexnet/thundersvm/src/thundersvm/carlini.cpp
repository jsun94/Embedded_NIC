...
// 	at::Tensor hidden_out1 = vi_svm_p->hidden_out.toTensor().detach().to({at::kCPU}).view({-1});
...
// 	at::Tensor hidden_out1 = pi_svm_p->hidden_out.toTensor().detach().view({1,-1});	/* detach */
...
// 		torch::jit::IValue hidden_out = forward_carlini(carlini, i);

// 		if(i == 1 || i ==3 || i == 4 || i == 6 || i == 8 || i == 9 || i == 10 || i == 12 || i == 14){
...
// 	//{
// 	// 	std::cout << "pi : "<< pi_result.size() << std::endl;
// 	// }
...
// 	std::cout << "layers output size : " << carlini->layers[14].output.toTensor().sizes() << "\n";
// }
...
// 		out = net->layers[idx].layer.forward(inputs);
