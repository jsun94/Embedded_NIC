...
#else
    std::ifstream bifs;
    bifs.open(file_name.c_str(), std::ios_base::binary);

    int size, size_h;
    bifs.read((char*)&size, sizeof(int));
    bifs.read((char*)&size_h, sizeof(int));
    vector<kernel_type> buf;
    for(int i=0; i<size; ++i){
        y_.emplace_back();
        instances_.emplace_back();
        bifs.read((char*)&y_.back(), sizeof(float_type));
        buf.resize(size_h);
        bifs.read(reinterpret_cast<char*>(&buf[0]), sizeof(kernel_type) * buf.size());
        for(int j=1; j<=size_h; ++j){
            instances_.back().emplace_back(j, buf[j-1]);
            //std::cout << instances_.back().back().index << ' ' << instances_.back().back().value << ' ';
        }
        buf.clear();
    }
    bifs.close();
    total_count_ = instances_.size();
    n_features_ = size_h;
#endif
