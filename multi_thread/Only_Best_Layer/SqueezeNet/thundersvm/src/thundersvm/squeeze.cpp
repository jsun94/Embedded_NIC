...
void *pi_job(DM_Pisvm *pi_svm_p){
	...
	// at::Tensor hidden_out1 = pi_svm_p->hidden_out.detach().view({100,-1});	/* when adv sample */
	...
	// at::Tensor current_out = pi_svm_p->derived_module.forward(de_inputs).toTensor().to({at::kCPU}).view({100,-1}); /* when adv sample */
	//저장
	...
	if(pi_svm_p->index_n != 0){
		...
		// predict_dataset.load_from_dense(100, 20, final_out, &label);	/* when adv sample */
