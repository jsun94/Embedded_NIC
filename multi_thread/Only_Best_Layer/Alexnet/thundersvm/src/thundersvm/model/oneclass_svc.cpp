...
//skycs
vector<float_type> OneClassSVC::predict_with_core(const DataSet::node2d &instances, int batch_size, int core_type, int n_core) {
    vector<float_type> dec_values = SvmModel::predict_with_core(instances, batch_size, core_type, n_core);
    vector<float_type> predict_y;
    for (int i = 0; i < dec_values.size(); ++i) {
        predict_y.push_back(dec_values[i] > 0 ? 1 : -1);
    }
