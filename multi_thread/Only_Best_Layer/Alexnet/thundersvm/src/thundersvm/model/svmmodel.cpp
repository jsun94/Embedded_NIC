...
using svm_kernel::sum_kernel_values_with_cpu;
...
void SvmModel::predict_dec_values(const DataSet::node2d &instances, SyncArray<float_type> &dec_values,
...
//skycs
void SvmModel::predict_dec_values_with_core(const DataSet::node2d &instances, SyncArray<float_type> &dec_values,
                             int batch_size, int core_type, int n_core) const {
    SyncArray<int> sv_start(n_classes);//start position of SVs in each class
    sv_start.host_data()[0] = 0;
    for (int i = 1; i < n_classes; ++i) {
        sv_start.host_data()[i] = sv_start.host_data()[i - 1] + n_sv.host_data()[i - 1];
    ...
    while (batch_end != instances.end()) {
        ...
        if(core_type == 0){
            ...
            sum_kernel_values_with_cpu(coef, sv.size(), sv_start, n_sv, rho, kernel_values, batch_dec_values, n_classes,
                          batch_ins.size(), n_core);
        }
...
//skycs gpu = 0 cpu = 1
vector<float_type> SvmModel::predict_with_core(const DataSet::node2d &instances, int batch_size=-1, int core_type=0, int n_core=1){
    dec_values.resize(instances.size() * n_binary_models);
//    vector<float_type> dec_values_vec(dec_values.size());
//    dec_values.set_host_data(dec_values_vec.data());
#ifdef USE_CUDA
...
#endif
    predict_dec_values_with_core(instances, dec_values, batch_size, core_type, n_core);
    dec_values.to_host();//copy back from device
    float_type* dec_values_host = dec_values.host_data();
    vector<float_type> dec_values_vec(dec_values.size());
    memcpy(dec_values_vec.data(), dec_values_host, dec_values.size() * sizeof(float_type));
...
#else
    ofstream bfs_model;
    bfs_model.open(path.c_str(), std::ios_base::out | std::ios_base::trunc | std::ios_base::binary);
    int count = 0;
    const float_type *coef_data = coef.host_data();
    vector<kernel_type> buffer;

    const SvmParam &param = this->param;
    bfs_model.write((const char*)&param.svm_type, sizeof(param.svm_type));
    bfs_model.write((const char*)&param.kernel_type, sizeof(param.kernel_type));
    bfs_model.write((const char*)&param.gamma, sizeof(param.gamma));
    bfs_model.write((const char*)&n_classes, sizeof(n_classes));
    int size = sv.size();
    bfs_model.write((const char*)&size, sizeof(size));
    float_type rho_v = rho.host_data()[0];
    bfs_model.write((const char*)&rho_v, sizeof(rho_v));
    int size_h = sv[0].size();
    bfs_model.write((const char*)&size_h, sizeof(size_h));
    for (int i = 0; i < sv.size(); i++) {
        for (int j = 0; j < n_classes - 1; ++j) {
            bfs_model.write((char*)(&coef_data[j*sv.size()+i]), sizeof(float_type));
            count++;
        }
        ...
        int k = 0;
        for (; k < p.size(); k++) {
            buffer.push_back(p[k].value);
            count++;
        }
        bfs_model.write(reinterpret_cast<const char*>(&buffer[0]), buffer.size()*sizeof(float));
        buffer.clear();
    }
    bfs_model.close();
#endif
...
void SvmModel::load_from_file(string path) {
    ...
    bifs.open(path.c_str(), std::ios_base::binary);

    bifs.read((char*)&param.svm_type, sizeof(param.svm_type));
    bifs.read((char*)&param.kernel_type, sizeof(param.kernel_type));
    bifs.read((char*)&param.gamma, sizeof(param.gamma));
    bifs.read((char*)&n_classes, sizeof(n_classes));
    bifs.read((char*)&size, sizeof(size));
    bifs.read((char*)&rho_v, sizeof(rho_v));
    bifs.read((char*)&size_h, sizeof(size_h));

    // LOG(INFO) << "svm_type " << param.svm_type << " kernel_type " << param.kernel_type << " gamma " << param.gamma << " nclasses " << n_classes << " size " << size << " rho_v " << rho_v << " size " << size_h;
    n_binary_models = n_classes * (n_classes - 1) / 2;
    rho.resize(n_binary_models);
    n_sv.resize(n_classes);
    rho.host_data()[0] = rho_v;
    n_total_sv = size;
    sv.clear();
    ...
    for(int i=0; i<n_total_sv; i++){
        sv.emplace_back();
        bifs.read((char *)&coef_data[i], sizeof(float_type));
        buffer.resize(size_h);
        bifs.read(reinterpret_cast<char*>(&buffer[0]), sizeof(kernel_type) * buffer.size());
        for(int j=1; j<=size_h; ++j){
            sv.back().emplace_back(j, buffer[j-1]);
        }
        buffer.clear();
    }
