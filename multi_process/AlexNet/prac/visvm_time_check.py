...
class Visvm(Process):
    ...
    def visvm_handler(self, signum, frame):
        ...
        # self.start_visvm.record()
        start_visvm = time.time()
        self.pre_result = self.model.predict_gpu(self.visvm_input)
        print(self.name, ' ', time.time()-start_visvm, ' sec')
        # self.end_visvm.record()
        # torch.cuda.synchronize()
        # print(self.name, " INFERENCE TIME : ", self.start_visvm.elapsed_time(self.end_visvm)/1000)
    ...
    def run(self):
        ...
        self.start_visvm = torch.cuda.Event(enable_timing = True)
        self.end_visvm = torch.cuda.Event(enable_timing = True)
